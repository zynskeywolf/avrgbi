...
inline void pix(unsigned char c, unsigned char x, unsigned char y) {
  ...
  scrnptr[i]=(x%2? scrnptr[i]&0xf0|(c&0x0f) : scrnptr[i]&0x0f|(c<<4));
}
...
void row(unsigned char c, unsigned char line, unsigned char x0, unsigned char x1)
{
  ...
  {
    scrnptr[line*_HBYTES+x0/2]=scrnptr[line*_HBYTES+x0/2]&0xf0|(c&0x0f);
    i++;
  }
  ...
  if(!(x1%2)) // right end on even pixel
    scrnptr[line*_HBYTES+x1/2]=scrnptr[line*_HBYTES+x1/2]&0x0f|(c<<4);
}

void col(unsigned char c, unsigned char col, unsigned char y0, unsigned char y1)
{
  if(col>=_HRES) return; // coloumn out of screen: do nothing
...
void rect(unsigned char c, unsigned char x0, unsigned char y0, unsigned char x1, unsigned char y1)
{
  ...
  for(unsigned char i=x0;i<=x1;i++) // many coloumns
  {
    col(c,i,y0,y1);
    if(debug) delay(32);
  }
...
void loop()
{
  ...
  cmd=Serial.read();
  if(debug) cmd%=5; // debug mode: modulo to make sure to react to any number
  switch(cmd)
  {
    case 0: // fill
